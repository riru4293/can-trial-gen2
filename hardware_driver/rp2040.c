...
void rp2040_init_spi_1( void )
{
    ...
    (void)spi_init( SPI_1, SPI_1_BAUDRATE );
...
void rp2040_turn_on_led_1( const bool is_on )
{
    const bool lv = ( true == is_on ) ? GPIO_LEVEL_HIGH : GPIO_LEVEL_LOW;
...
void rp2040_read_spi_1_array( const size_t n, uint8_t buff[ n ] )
{
    if( ( 0U < n ) && ( NULL != buff ) )
    {
        (void)spi_read_blocking( SPI_1, SPI_1_REPEATED_TX_DATA, buff, n );
    }
}

void rp2040_write_spi_1_array( const size_t n, const uint8_t buff[ n ] )
{
    if( ( 0U < n ) && ( NULL != buff ) )
    {
        (void)spi_write_blocking( SPI_1, buff, n );
    }
}
...
uint8_t rp2040_read_spi_1( void )
{
    ...
    rp2040_read_spi_1_array( sizeof( val ), &val );
...
{
    rp2040_write_spi_1_array( sizeof( val ), &val );
}
